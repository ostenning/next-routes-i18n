...
class Routes {
  ...
  findAndGetHref (name, locale, params) {
    locale = locale || this.locale
    const route = this.findByName(name, locale) || this.routes[0]
    
    if (route) {
      return route.getHref(params);
    }
  ...
  getLink (Link) {
    return props => {
      ...
      const hrefValue = this.findAndGetHref(href, locale2, params);
      Object.assign(newProps, { href: hrefValue })
  ...
  getRouter (Router) {
    const wrap = method => (route, params, locale, options) => {
      const href = this.findAndGetHref(route, locale, params)
      return Router[method](href, null, options)
    }
...
class Route {
  ...
  getHref (params = {}) {
    let href = (this.hideLocale ? '' : '/' + this.locale) + this.toPath(params)
    ...
    if (!qsKeys.length) return href

    const qsParams = qsKeys.reduce((qs, key) => Object.assign(qs, {
      [key]: params[key]
    }), {})

    return `${href}?${toQuerystring(qsParams)}`
  }
}
